...
/**
 * Check all blocks and read jsons & deps then sort levels and write google-map.
 *
 * @param {Object} task
 *
 * @return {undefined}
...
module.exports = function(task) {
  ...
  try {
      ...
      fs.readdirSync(paths.blocks(level)).forEach(block => {
        ...
        if (writeMap)
          fs.readdirSync(paths.blocks(level, block)).forEach(file => {
            ...
            file = path.join(
              path.relative(path.dirname(mapPath), blocks),
              level,
              block,
              file
            );
        ...
        if (isFile(json)) {
          try {
            ...
            throw new Error(
              `\n\n\x1b[41mFAIL\x1b[0m: A JSON "\x1b[36m${path.join(
                level,
                block,
                'data.json'
            ...
            if (data)
              deps[block] = {
                nodes: Array.isArray(data.nodes)
                  ? deps[block].nodes.concat(data.nodes)
                  : deps[block].nodes,
                modules: Array.isArray(data.modules)
                  ? deps[block].modules.concat(data.modules)
                  : deps[block].modules,
              };
          }
        }
    ...
    if (writeMap) {
      if (!isDirectory(path.dirname(mapPath)))
