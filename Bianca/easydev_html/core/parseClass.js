...
module.exports = function(cls, page, mix, attrs, tag) {
  ...
  if (!page.nodes[node])
    page.nodes[node] = { name: node, mod: [], mix: [], attrs, tag };

  if (isModifier) {
    if (page.nodes[node].mod.indexOf(cls) === -1)
      page.nodes[node].mod.push(cls);
  } else {
    if (mix && mix.length > 0) {
      mix.forEach(item => {
        if (page.nodes[node].mix.indexOf(item) === -1)
          page.nodes[node].mix.push(item);
      });
    }
