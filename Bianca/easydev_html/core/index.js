...
const paths = {
  ...
  lPage() {
    return path.join(this._lPage, ...arguments);
  },
  ...
  _lPage: path.join(root, 'app', 'landing'),
};
...
try {
  ...
  if (config.use.templates[0] !== '.')
    config.use.templates = '.' + config.use.templates;
  if (config.use.scripts[0] !== '.')
    config.use.scripts = '.' + config.use.scripts;
  ...
  if (config.HTMLBeautify !== false)
    config.HTMLBeautify = Object.assign(beautify, config.HTMLBeautify);

  // Add dist paths

  for (let key in config.dist) {
    if (!paths[`_${key}`])
      paths[`_${key}`] = path.join(
        paths._dist,
        (config.dist[key] || '').trim()
      );
  }
}
