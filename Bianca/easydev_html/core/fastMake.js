...
module.exports = {
  ...
  addNode(node, extra) {
    ...
    extra.forEach(item => {
      ...
      let name =
        path.basename(item, extname) || (extname === '.json' ? 'data' : node);
      let content = (
      ...
      if (name + extname === 'deps.js')
        content = (config.addContent && config.addContent.deps) || '';
  ...
  parseArguments(argv, showMessage = true) {
    ...
    if (this.items.length === 0) {
      this.status = false;
      this.message = `\x1b[41mFAIL\x1b[0m: You must write a \x1b[36m${
        this.type
      }\x1b[0m name!`;
    } else {
      try {
        this.items.forEach(item => {
          ...
          if (config.fastMake && config.fastMake[more])
            extra = config.fastMake[more];
